...
상태 갱신: 2026-09-18 KST — 기존 DB 기반 연관어 대체 집계와 축별 정렬, 긍부정 신호 근거 팝업을 구현하고 최신 `main`과 별도 통합 브랜치에서 검증 중입니다. 아래의 "구현 중" 표현은 최초 작성 당시의 기록이며 배포 완료를 뜻하지 않습니다.

기준 브랜치: `main`
기준 커밋: `cc57ae6` (`살말 댓글 모델과 0054 마이그레이션 동기화`)

최초 작업 브랜치: `feat/assoc-existing-data`

최초 작성 당시 상태: 문서와 직접 연결 fast path 구현 중, 아직 commit/push하지 않음
