...
function trFillBarsV() {
  ...
  if (HAS_A) {
    aUtils.set(fl, { height: '0%' });
    aAnimate(fl, {
      height: el => el.dataset.h + '%', duration: 1050, delay: aStagger(90),
...
export function trRender(id) {
  ...
  /* ══════════════ 언급량 · 온도 ══════════════
    ...
    G_CFG.tempMain = {
      key: kw + 'temp', term: kw, min: 0, max: 100,
  ...
  /* ══════════════ 연관어 ══════════════
    ...
    G_CFG.assocMain = {
      key: kw + 'assoc', term: kw, rows: A.history || [],
  ...
  /* ══════════════ 할인률 변화 ══════════════
    ...
    if (TR_TAB === '통합') {
      body.innerHTML =
        ...
        D.platforms.map((s, i) => {
          const top = Math.max.apply(null, D.platforms.map(x => x.avg_discount || 0)) || 1;
      ...
      G_CFG.stockPlat = {
        key: full + TR_TAB, rows, min: 0, max: 100,
  ...
  /* ══════════════ 리세일 시세 지수 ══════════════
    ...
    const tempRows = (TB.series || []).map(p => ({ date: p.date, temp: p.temp }));
    G_CFG.resMain = {
      key: full + 'res', rows: trMergeRows(D.series, tempRows),
...
function sentFitSignals() {
  ...
  let used = padV;
  card.querySelectorAll(':scope > *').forEach(ch => {
    if (ch === wrap) return;
  ...
  const setTo = (wrapH, cardH, animated) => {
    ...
    if (HAS_A && animated) {
      ...
      aAnimate(card, {
        height: [curC, cardH], duration: 380, ease: 'out(3)',
...
/* 토글 기능은 이제 각 카테고리 패널 내부에서 클라이언트 사이드로 처리됩니다 */
